#!/usr/bin/env sh
set -eu

export SPRING_PROFILES_ACTIVE="${SPRING_PROFILES_ACTIVE:-docker}"
export EUREKA_CLIENT_SERVICEURL_DEFAULTZONE="${EUREKA_CLIENT_SERVICEURL_DEFAULTZONE:-http://127.0.0.1:8761/eureka/}"
export USUARIOS_SERVICE_URL="${USUARIOS_SERVICE_URL:-http://127.0.0.1:8084}"
export REPORTES_SERVICE_URL="${REPORTES_SERVICE_URL:-http://127.0.0.1:8081}"
export REPORTS_SERVICE_URL="${REPORTS_SERVICE_URL:-http://127.0.0.1:8081/api/reportes/enviar}"
export GEO_SERVICE_URL="${GEO_SERVICE_URL:-http://127.0.0.1:8083}"
export ALERTAS_SERVICE_URL="${ALERTAS_SERVICE_URL:-http://127.0.0.1:8082}"

echo "Starting Eureka..."
java -jar /app/eureka-service.jar &
sleep 12

echo "Starting backend services..."
java -jar /app/ms-usuarios.jar &
java -jar /app/ms-reportes.jar &
java -jar /app/ms-geolocalizacion.jar &
java -jar /app/ms-alertas.jar &
sleep 18

echo "Starting API Gateway..."
exec java -jar /app/api-gateway.jar
